"""
generar_web_jumbo.py
====================
Lee los JSONs en data/ y genera docs/index.html para GitHub Pages.
Diseño inspirado en Jumbo: verde (#007a33) sobre fondo oscuro.
"""
...
def agrupar_graficos(graficos):
    ...
    for periodo, datos in graficos.items():
        ...
        if not cats_raw and serie_total:
            ...
            for cat in ORDEN_CATS:
                if cat in cats_raw:
                    cats_ordenadas[cat] = cats_raw[cat]
...
def main():
    ...
    graficos_agrupados = agrupar_graficos(graficos)
    ...
    cats_dia_ord = sorted(cats_dia, key=lambda x: orden.get(x.get("categoria", ""), 999))
    ...
    filas_cats = ""
    for cat in cats_dia_ord:
        pct   = cat.get("variacion_pct_promedio", 0)
        color = color_pct(pct)
        s     = "+" if pct > 0 else ""
        n_sube  = cat.get("productos_subieron", 0)
        n_baja  = cat.get("productos_bajaron", 0)
        n_total = cat.get("total_productos", 0)
        filas_cats += f"""
        <tr>
          <td>{cat['categoria']}</td>
          <td style="color:{color};font-weight:700;font-family:'IBM Plex Mono',monospace">{s}{pct:.2f}%</td>
          <td style="color:#ef4444">{n_sube}</td>
          <td style="color:#22c55e">{n_baja}</td>
          <td style="color:#888">{n_total}</td>
        </tr>"""
...
<title>Jumbo Price Tracker – Seguimiento diario de precios</title>
...
<header>
  <div class="logo-area">
    <div class="jumbo-badge">🐘 JUMBO</div>
    <div>
      <h1>PRICE TRACKER</h1>
...
<div class="container">
  <div class="info-banner">
    📊 Datos relevados automáticamente de <strong>jumbo.com.ar</strong> vía GitHub Actions.
    Los precios pueden variar según sucursal y disponibilidad. No afiliado con Cencosud/Jumbo.
  </div>
...
<footer>
  Datos relevados de jumbo.com.ar · Actualización automática diaria via GitHub Actions<br>
  JUMBOBOT no está afiliado con Cencosud S.A. · Uso educativo y de transparencia de precios
</footer>
...
function renderTablaRanking(id, data, esBaja) {{
  ...
  if (!data.length) {{
    tbody.innerHTML = '<tr><td colspan="4" style="color:var(--muted);text-align:center;padding:1rem">Sin datos aún — el tracker necesita al menos 2 días de datos</td></tr>';
    return;
  }}
